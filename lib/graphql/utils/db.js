...
export function getVersion (r, type, args) {
  ...
  if (args.id) {
    ...
    if (!args.recordId) returnList = true
    else filter._metadata.recordId = args.recordId
    q = q.filter(filter)
  } else if (args.date) {
    q = q.filter((rec) => {
      ...
      if (!args.recordId) returnList = true
      else subq = subq.and(rec('_metadata')('recordId').eq(args.recordId))
      return subq
    })
  } else {
    returnList = true
  ...
  return q.run().then(function (records) {
    if (!returnList) {
