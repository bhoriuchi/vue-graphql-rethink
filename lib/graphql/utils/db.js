...
export function getVersion (r, type, args) {
  ...
  if (args.id) {
    ...
    let time = r.ISO8601(toISO(args.date))
    q = q.filter(function (rec) {
      return rec('_metadata')('recordId').eq(args.recordId)
        .and(rec('_metadata')('validFrom').ne(null))
        .and(
          time.during(
    ...
    throw ERR_VERSION_NOT_FOUND()
  }
  return q.run().then(function (record) {
    let isArray = _.isArray(record)
    if (isArray && record.length > 0) record = record[0]
    else if (!record || (isArray && record.length === 0)) throw ERR_VERSION_NOT_FOUND()
    return record
  })
}
